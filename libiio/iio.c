...
// silly stuff from llibertat.h {{{1
...
//static void iio_image_separate_channels(struct iio_image *x)
//{
//	if (x->pixel_dimension < 2) error("image is gray!");
//	if (x->contiguous_data) error("do the right stuff here");
//	if (x->dimension + 1 >= IIO_MAX_DIMENSION) error("too many dimensions");
//	struct iio_image y[1];
//	*y = *x;
//	y->data = xmalloc(
//}
...
static void fill_struct_2d(struct iio_image *x, int w, int h, int nc, int type,
		void *data)
{
	x->dimension = 2;
	x->type = type;
	x->pixel_dimension = nc;
	x->data = data;
	x->sizes[0] = w;
	x->sizes[1] = h;
}
...
static void iio_hacky_colorize(struct iio_image *x, int nc)
{
	assert(!x->contiguous_data);
	...
	void *rdata = xmalloc(n*ss*nc);
	char *data_dest = rdata;
	char *data_src = x->data;
	FORI(n)
		FORJ(nc)
			memcpy(data_dest + (nc*i + j)*ss, data_src + i*ss, ss);
	xfree(x->data);
	x->data=rdata;
	x->pixel_dimension = nc;
}
		...
		FILE *f, char *header, int nheader)
{
	...
	int w, h, nc = isupper(header[2]) ? 3 : 1;
	float scale;
	if (!isspace(pilla_caracter_segur(f))) return -1;
	if (3 != fscanf(f, "%d %d\n%g", &w, &h, &scale)) return -2;
	if (!isspace(pilla_caracter_segur(f))) return -3;
	float *data = xmalloc(w*h*4*nc);
	if (1 != fread(data, w*h*4*nc, 1, f)) return -4;
	if (scale < 0) switch_4endianness(data, w*h*nc);
...
static void iio_save_image_as_png(const char *filename, struct iio_image *x)
{
	...
	int ss = bit_depth/8;
	int nc = x->pixel_dimension;
	png_bytepp row = xmalloc(height * sizeof(png_bytep));
	FORI(height) row[i] = i*nc*width*ss + (uint8_t *)x->data;
...
// API 2D
uint16_t *iio_read_image_uint16_vec(const char *fname, int *w, int *h, int *nc)
{
	struct iio_image x[1];
	int r = read_image(x, fname);
	if (r) return rerror("could not read image");
	       	...
	       	int *w, int *h, int *nc)
{
	...
	*h = x->sizes[1];
	*nc = x->pixel_dimension;
	fprintf(stderr, "matrix_vec nc = %d\n", *nc);
	//iio_save_image_default("/tmp/iioo_before", x);
	iio_convert_samples(x, IIO_TYPE_UINT8);
	//iio_save_image_default("/tmp/iioo_after", x);
	return wrap_2dmatrix_around_data(x->data, *w, *h, *nc);
}
...
{
	return iio_read_image(fname, w, h, IIO_TYPE_FLOAT);
}
...
static void iio_save_image_default(const char *filename, struct iio_image *x)
{
	...
	if (true) {
		...
		if (tiffname == filename) {
			iio_save_image_as_tiff(filename+5, x);
			return;
		}
	}
	if (true) {
