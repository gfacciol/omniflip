...
// metadata.  The "vec" functions produce the data in the same order as is
// stored.)
uint8_t ***iio_read_image_uint8_matrix_vec(const char *fnam, int *w, int *h, int *nc);
...
void iio_save_image_float_vec(char *filename, float *x, int w, int h, int nc);
void iio_save_image_uint8_vec(char *filename, uint8_t *x, int w, int h, int nc);
void iio_save_image_uint16_vec(char *filename, uint16_t *x, int w, int h, int nc);
